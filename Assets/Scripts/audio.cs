	...
	private void FixedUpdate()
		{
			...
			float camDist = (MCamera.transform.position - transform.position).sqrMagnitude;
			...
			if (m_StartedSound)
			{
				...
				float pitch = ULerp(lowPitchMin, lowPitchMax, VCars.engineRPM / VCars.maxRPM);
				...
				// get values for fading the sounds based on the acceleration
					...
					accFade = Mathf.Abs((VCars.verInput > 0 && !VCars.test ) ?VCars.verInput : 0);

					float decFade = 1 - accFade;

					// get the high fade value based on the cars revs
					float highFade = Mathf.InverseLerp(0.2f, 0.8f,  VCars.engineRPM / 10000);
